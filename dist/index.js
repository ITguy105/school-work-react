...
async function listAssignments() {
  const db = await getDb();
  if (!db) return seedAssignments.map((item, index) => ({ ...item, id: index + 1, createdAt: /* @__PURE__ */ new Date() }));
...
var appRouter = router({
  ...
  assignments: router({
    ...
    create: publicProcedure.input(z2.object({
      ...
      dueDate: z2.string().min(1).max(32),
      priority: z2.enum(["low", "medium", "high"])
    })).mutation(({ input }) => createAssignment({ ...input, completed: 0 })),
    toggle: publicProcedure.input(z2.object({ id: z2.number().int().positive() })).mutation(({ input }) => toggleAssignment(input.id)),
    remove: publicProcedure.input(z2.object({ id: z2.number().int().positive() })).mutation(({ input }) => deleteAssignment(input.id))
  })
});
